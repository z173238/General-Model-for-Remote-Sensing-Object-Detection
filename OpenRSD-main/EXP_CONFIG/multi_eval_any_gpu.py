...
if __name__ == "__main__":
    ...
    for model_name, cfg in cfgs.items():
        ...
        if len(white_list) != 0 and not has_attr(model_name, white_list):
            continue
        if not os.path.exists(cfg['cp_file']):
            cp_file = cfg['cp_file']
            print(f'Pass {model_name}, checkpoints {cp_file} not exists')
            continue
        if os.path.exists(cfg['submission_dir']):
            print(f'{model_name} submit dir exists, Ignore')
            continue
        gpu_nums.append(1)
        cmds.append('python py_cmd.py %s -m test -port %d -addr %s'
                    % (model_name, 29500 +
                       cmd_count,
