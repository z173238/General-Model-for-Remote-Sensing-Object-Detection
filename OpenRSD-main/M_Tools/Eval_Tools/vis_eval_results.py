...
def plot_curves(x, ys, ls,
    ...
    fig_all, ax = get_ax_obj(save_path, figsize=(8, 8))
    for y, l, c in zip(ys, ls, cs):
        ax.plot(x, y, '-.', label=l, color=c)

    for y in ys:
        for i, value in enumerate(y):
            plt.text(x[i], value + 0.001, str(value), ha='center', va='bottom')
...
if __name__ == '__main__':
    ...
    """
    eval_root = '/data/space2/huangziyue/mmdet_results/TEST_EVAL/'\
                 'Formal_12_11_A10_flex_rtm_v3_1_formal_ablation_balance'
    fig_path = './Formal_12_11_A10_flex_rtm_v3_1_formal_ablation_balance.png'
    eval_results = dict()
    # ---- 每个Epoch
