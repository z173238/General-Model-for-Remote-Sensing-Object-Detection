...
class DynamicSoftLabelAssigner(BaseAssigner):
    ...
    def assign(self,
        ...
        cls_cost_shape = num_valid * num_gt * pred_scores.shape[-1]

        # if pred_scores.shape[-1] >= 40 and num_valid * num_gt > 12000 * 240:
        #     device = gt_labels.device
        #     gt_onehot_label = (
        #         F.one_hot(gt_labels.to(torch.int64),
        #                   pred_scores.shape[-1]).float().unsqueeze(0).repeat(
        #                       num_valid, 1, 1)).detach().cpu()
        #     valid_pred_scores = valid_pred_scores.unsqueeze(1).repeat(1, num_gt, 1).detach().cpu()
        #     print('Two many class, gt_onehot_label.shape: ', gt_onehot_label.shape)
        #     soft_label = gt_onehot_label * pairwise_ious[..., None].detach().cpu()
        #     scale_factor = soft_label - valid_pred_scores.sigmoid()
        #     soft_cls_cost = F.binary_cross_entropy_with_logits(
        #         valid_pred_scores, soft_label,
        #         reduction='none') * scale_factor.abs().pow(2.0)
        #     soft_cls_cost = soft_cls_cost.sum(dim=-1).to(device)
        gt_onehot_label = (
            F.one_hot(gt_labels.to(torch.int64),
                      pred_scores.shape[-1]).float().unsqueeze(0).repeat(
                num_valid, 1, 1))
