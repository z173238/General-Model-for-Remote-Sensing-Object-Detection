...
class ResidualMLP(nn.Module):
    ...
    def forward(self, x):
        x_out = self.fc_out(x) + self.mlp_residual(x)
        return x_out
...
class ContrastiveEmbed(BaseClsHead):
    ...
    def forward(self,
        ...
        if align_style in ['labelled', 'pure_img', 'img_text']:  # ---- support是visual embeds
            bias = self.bias_t1
...
class OpenRotatedRTMDetSepBNHeadSimpleBase(RotatedRTMDetSepBNHead):
    """
    提高并行性，增加推理速度
    ...
    def __init__(self,
        ...
        self.visual_fc = MLP(embed_dims, 2048, 256, 2)
        # --------- 显式的对齐
    ...
    def loss(self,
        ...
        losses['loss_cls'][0] = (losses['loss_cls'][0] +
                                 0.0 * sum(x.view(-1)[0] for x in self.text_fc.parameters()) +
                                 0.0 * sum(x.view(-1)[0] for x in self.visual_fc.parameters()))
        if not return_outs:
            return losses
        else:
            return losses, outs
    ...
    def forward(self,
        ...
        out_support_feats = support_feats
        ...
        for idx, (x, stride) in enumerate(
            ...
            ###################### 计算匹配分数，num_in_classes为n_class + sam_object，因此要减1
            # ---- pred_embed为原始的视觉特征，而cls_logit则是pred_embed和out_support_feats都经过“语义映射”之后计算的相似性logit
            # ---- 因此cls_logit可以实现精确的语义。
    ...
    def _get_targets_single(self,
        ...
        return (anchors, labels, ins_labels, label_weights, bbox_targets, assign_metrics,
                sampling_result)
    ###################################### ----- 获取检测器自己预测的embeddings的过程

    def forward_wo_supports(self,
                            feats: Tuple[Tensor, ...],
                            **kwargs) -> tuple:
        """
        forward获得bbox_preds和embeddings predictions
        :param feats:
        :param kwargs:
        :return:
        """
        ...
        for idx, (x, stride) in enumerate(
            ...
            angle_preds.append(angle_pred)
            pred_embeds.append(pred_embed)
        return bbox_preds, angle_preds, pred_embeds

    def get_pos_embeds(self,
                       cls_scores: List,
                       bbox_preds: List,
                       angle_preds: List,
                       embed_preds: List,
                       ...
                       batch_gt_instances_ignore: OptInstanceList = None):
        """
        根据gt_instances和检测结果，匹配获得Positive embeddings
        :param cls_scores:
        :param bbox_preds:
        :param angle_preds:
        :param embed_preds:
        :param batch_gt_instances:
        :param batch_img_metas:
        :param batch_gt_instances_ignore:
        :param obj_embeds:
        :param obj_labels:
        :param support_feats:
        ...
        ########### 设置虚拟的cls_scores，如果没有的话
        if cls_scores == None or len(cls_scores) == 0:
            cls_scores = []
            for box in bbox_preds:
                B, D_box, H, W = box.shape
                score = torch.ones([B, 1, H, W]).to(device) * 0.0
                cls_scores.append(score)
            for gt_instances in batch_gt_instances:
                gt_instances.labels = gt_instances.labels * 0
        ########### 设置ins_label，如果没有的话
        start = 0
        for i, gt_instances in enumerate(batch_gt_instances):
            if hasattr(gt_instances, 'ins_labels'):
                continue
            bboxes = gt_instances.bboxes
            ins_labels = torch.arange(start, start + len(bboxes)).to(device).long()
            gt_instances['labels'] = ins_labels
            start += len(bboxes)
        ...
        anchor_list, valid_flag_list = self.get_anchors(
            featmap_sizes, batch_img_metas, device=device)
        cls_out_channels = bbox_preds[0].shape[1]
        flatten_cls_scores = torch.cat([
            cls_score.permute(0, 2, 3, 1).reshape(num_imgs, -1,
                                                  cls_out_channels)
            for cls_score in bbox_preds
        ], 1)
        ...
        (anchor_list, labels_list, ins_labels_list, label_weights_list, bbox_targets_list,
         assign_metrics_list, sampling_results_list) = cls_reg_targets
        ########### 获得Positive embeddings, boxes以及对应的实例标签
        all_pred_embeds = torch.cat([
            embed.permute(0, 2, 3, 1).reshape(num_imgs, -1,
                                              self.embed_dims)
            for embed in embed_preds
        ...
        # ----- n_level x [(B, H_i*W_i)] -> B x n_position
        all_labels = torch.cat(labels_list, dim=1)
        all_ins_labels = torch.cat(ins_labels_list, dim=1)


        all_pos_pred_embeds = []
        all_pos_pred_boxes = []
        all_pos_pred_ins_labels = []
        all_pos_pred_sem_embeds = []

        for i in range(num_imgs):
            assigned_labels = all_labels[i]
            assign_ins_labels = all_ins_labels[i]
            pred_embeds = all_pred_embeds[i]
            pred_boxes = flatten_bboxes[i]
            ...
            # ----- Positive的预测embeddings
            pos_pred_embeds = pred_embeds[pos_inds]
            pos_ins_labels = assign_ins_labels[pos_inds]
            pos_pred_boxes = pred_boxes[pos_inds]
            pos_pred_sem_embeds = self.visual_fc(pos_pred_embeds)

            all_pos_pred_embeds.append(pos_pred_embeds)
            all_pos_pred_boxes.append(pos_pred_boxes)
            all_pos_pred_ins_labels.append(pos_ins_labels)
            all_pos_pred_sem_embeds.append(pos_pred_sem_embeds)

        return batch_gt_instances, all_pos_pred_embeds, all_pos_pred_boxes,\
            all_pos_pred_ins_labels, all_pos_pred_sem_embeds
############################################################### 预测相关
...
############################################################### 预测相关
    def predict_with_embed(self,
        ...
        outs = outs[:-1]
        ###############
        predictions = self.predict_by_feat_with_embed(
            *outs, batch_img_metas=batch_img_metas, rescale=rescale)
        return predictions

    def predict_by_feat_with_embed(self,
                        cls_scores: List[Tensor],
                        bbox_preds: List[Tensor],
                        angle_preds: List[Tensor],
                                   embed_preds,
        ...
        for img_id in range(len(batch_img_metas)):
            ...
            angle_pred_list = select_single_mlvl(
                angle_preds, img_id, detach=True)
            embed_pred_list = select_single_mlvl(
                embed_preds, img_id, detach=True)
            if with_score_factors:
                score_factor_list = select_single_mlvl(
                    score_factors, img_id, detach=True)
            else:
            ...
            results = self._predict_by_feat_single_with_embed(
                cls_score_list=cls_score_list,
                bbox_pred_list=bbox_pred_list,
                angle_pred_list=angle_pred_list,
                embed_pred_list=embed_pred_list,
                score_factor_list=score_factor_list,
                mlvl_priors=mlvl_priors,
                img_meta=img_meta,
                cfg=cfg,
    ...
    def _predict_by_feat_single_with_embed(self,
                                cls_score_list: List[Tensor],
                                bbox_pred_list: List[Tensor],
                                angle_pred_list: List[Tensor],
                                           embed_pred_list,
        ...
        if with_score_factors:
            ...
            mlvl_score_factors = None
        for level_idx, (
                cls_score, bbox_pred, angle_pred, embed_pred, score_factor, priors) in \
                enumerate(zip(cls_score_list, bbox_pred_list, angle_pred_list,
                              embed_pred_list,
                              score_factor_list, mlvl_priors)):
            ...
            angle_pred = angle_pred.permute(1, 2, 0).reshape(
                -1, self.angle_coder.encode_size)
            embed_pred = embed_pred.permute(1, 2, 0).reshape(
                -1, 256)
            if with_score_factors:
                score_factor = score_factor.permute(1, 2,
                                                    0).reshape(-1).sigmoid()
            ...
            angle_pred = filtered_results['angle_pred']
            priors = filtered_results['priors']
            embed_pred = filtered_results['embed_pred']
            ...
            mlvl_scores.append(scores)
            mlvl_labels.append(labels)
            mlvl_embed_preds.append(embed_pred)
        ...
        priors = cat_boxes(mlvl_valid_priors)
        bboxes = self.bbox_coder.decode(priors, bbox_pred, max_shape=img_shape)
        embeds = torch.cat(mlvl_embed_preds)
        sem_embeds = self.visual_fc(embeds)

        results = InstanceData()
        results.bboxes = RotatedBoxes(bboxes)
        results.scores = torch.cat(mlvl_scores)
        results.labels = torch.cat(mlvl_labels)
        results.embeds = embeds
        results.sem_embeds = sem_embeds
        if with_score_factors:
            results.score_factors = torch.cat(mlvl_score_factors)
