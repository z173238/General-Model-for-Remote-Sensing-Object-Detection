...
class DOTADatasetClamp(BaseDataset):
        ...
        if self.ann_file == '':
            ...
            if len(txt_files) == 0:
                raise ValueError('There is no txt file in '
                                 f'{self.ann_file}')
            for txt_file in tqdm(list(txt_files)):
                data_info = {}
                ...
                with open(txt_file) as f:
                    # s = f.readlines()
                    s = list(f.readlines())
                    if len(s) >= self.max_instance:
                        print(f'Warning: too many instances in {txt_file}, cut {len(s)} to {self.max_instance}')
                        s = np.random.permutation(np.array(s))[:self.max_instance].tolist()
