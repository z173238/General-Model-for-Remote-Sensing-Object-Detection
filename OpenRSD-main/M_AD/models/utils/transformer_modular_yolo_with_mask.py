...
class TwoWayAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        if self.with_query_self_attn:
            q = queries
            attn_out = self.self_attn(q=q, k=q, v=q, mask=query_masks)
        ...
        if self.with_key_self_attn:
            q = keys
            attn_out = self.self_attn_k(q=q, k=q, v=q, mask=key_masks)
            keys = keys + attn_out
        ...
        if self.with_cross_key_to_query:
            q = queries
            k = keys
            attn_out = self.cross_attn_token_to_image(q=q, k=k, v=keys, mask=key_masks)
            queries = queries + attn_out
        ...
        if self.with_cross_query_to_key:
            q = queries
            k = keys
            attn_out = self.cross_attn_image_to_token(q=k, k=q, v=queries, mask=query_masks)
            keys = keys + attn_out
...
class Attention(nn.Module):
    ...
    def forward(self, q: Tensor, k: Tensor, v: Tensor, mask: Tensor) -> Tensor:
        ...
        # Separate into heads: B x N x C -> B x N_heads x N x C_per_head
        ...
        # 按照mask，将对应位置key的Attention置为-inf，softmax之后为0
        if mask is not None:
            # B x N_k -> B x N_heads x N_q x N_k
            mask = mask[:, None, None, :].expand_as(attn)
            attn = attn.masked_fill(mask == 0, float('-inf'))

        attn = self.softmax(attn, dim=-1)

        # print('PRINT_attn in Transformer', attn.shape, attn[0, 0, 0, :])
