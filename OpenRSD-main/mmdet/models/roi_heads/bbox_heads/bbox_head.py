    ...
    def loss(self,
        ...
        if bbox_pred is not None:
            ...
            if pos_inds.any():
                ...
                # n_pred_nan = len(pos_bbox_pred[torch.isnan(pos_bbox_pred)])
                # pos_bbox_targets = bbox_targets[pos_inds.type(torch.bool)]
                # n_target_nan = len(pos_bbox_targets[torch.isnan(pos_bbox_targets)])
                # print(f'NaN, Pred:{n_pred_nan}, Target: {n_target_nan}')
                # if torch.sum(torch.isnan(losses['loss_bbox'])) > 0:
                #     a = 0
