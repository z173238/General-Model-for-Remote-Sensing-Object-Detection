...
class MMDetSafeDynamicSoftLabelAssigner(BaseAssigner):
    """
    防止显存溢出
    ...
    def assign(self,
        ...
        prob = valid_pred_scores.sigmoid()
        C = valid_pred_scores.shape[-1]
        gt_eye = torch.eye(C).to(device)
        eps = 1e-8
        # N * C
        gt_cls_cost = (-torch.log(prob + eps) * (1 - prob) ** 2) @ gt_eye + \
                      (-torch.log(1 - prob + eps) * (prob) ** 2) @ (1 - gt_eye) + 0.5
        # M * C
        gt_onehot_mat = F.one_hot(gt_labels.to(torch.int64), C).float()
        # N * M
        soft_cls_cost = gt_cls_cost @ gt_onehot_mat.T
        #############################################
