...
if __name__ == '__main__':
    from M_Tools.Base_Data_infos.OOD_STAR_data_infos import data_infos
    ...
    # model_info = dict(
    #     val_using_aux=False,
    #     cfg_pth='G03_OpenBaselines/G03_Open_ORCNN_Full.py',
    #     cfg_name='G03_Open_ORCNN_Full',
    #     epoch=12,
    #     support_type='text',
    # )
    model_info = dict(
        val_using_aux=False,
        cfg_pth='G03_OpenBaselines/G03_Open_PKINet_Full.py',
        cfg_name='G03_Open_PKINet_Full',
        epoch=12,
        support_type='text',
    )
    ...
    support_type = model_info['support_type']
    work_root = f'./results/TEST_EVAL/STAR_25_3_8_{cfg_name}'
    mkdir(work_root)
    for epoch in epochs:
        print(model_info)
        ...
        for data_name in data_infos.keys():
            if data_name not in ['Data28_STAR',]:
                continue

            cfg = data_infos[data_name]
            results_pth = f'{results_dir}/{data_name}_{support_type}.pkl'
            eval_pth = f'{results_dir}/{data_name}_{support_type}.json'
            main(tst_cfg_pth,
