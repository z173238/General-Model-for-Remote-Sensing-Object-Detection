...
def parse_args():
    ...
    parser.add_argument('-type', help='epochs')
    parser.add_argument('-n', help='num of prompts', nargs='+')
...
n_prompts = [int(n) for n in args.n]
print('#' * 100)
print(f'support_type: {support_type}, n_prompts: {n_prompts}, gpu: {gpu}')
...
def main(tst_cfg_pth, ckpt_pth, work_dir, results_pth, eval_pth,
    ...
    cfg.model.support_type = support_type
    cfg.model.num_val_prompts = num_val_prompts
...
if __name__ == '__main__':
    ...
    work_root = f'./results/TEST_EVAL/25_1_17_EvalDiffPrompts_{cfg_name}'
    mkdir(work_root)
    for num_p in n_prompts:
        print(model_info)
        tst_cfg_pth = f'./M_configs/{cfg_pth}'
        # A08_e_rtm_v2_base_recheck
        ckpt_pth = f'./results/MMR_AD_{cfg_name}/epoch_{24}.pth'
        work_dir = f'{work_root}/Eval_Logs_Epoch_{24}'
        eval_results_root = f'{work_root}/Formal_Results_Epoch_{24}'
        if val_using_aux:
        ...
        for data_name in data_infos.keys():
            if data_name not in ['Data1_DOTA1',
                                 'Data11_WHU_Mix']:
                continue

            cfg = data_infos[data_name]
            results_pth = f'{results_dir}/{data_name}_{support_type}_NumP{num_p}.pkl'
            eval_pth = f'{results_dir}/{data_name}_{support_type}_NumP{num_p}.json'
            main(tst_cfg_pth,
