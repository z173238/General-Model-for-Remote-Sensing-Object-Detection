...
def main(data_root, img_dir, ann_dir, class_name, batch_size):
    epoch = 36
    ...
    iou_thr = 0.5
    out_ann_dir = f'{data_root}/Step1_Trans_HBB2OBB'
    ...
    with torch.no_grad():
        for data_info in data_loader:
            ...
            avg_time = used_time / max(count, 1)
            eta_time = (n_total - count) * avg_time
            print(f'{count} / {n_total}, used: {used_time}, time_per_img: {avg_time}. eta: {eta_time}')
            ...
            for sample in data_samples:
                ...
                out_ann_file_pth = out_ann_dir + '/' + img_name + '.txt'
                if not os.path.exists(out_ann_file_pth):
                    all_processed = False
            if all_processed:
                print(f'Pass Processed')
                count += batch_size
                continue
            ...
            all_results_list, out_proposals_list = model.predict(data['inputs'],
                                                                 ...
                                                                 sample_time=64,
                                                                 noise_scale=0.1)
            for batch_idx in range(len(data['data_samples'])):
                count += 1
                sw1, sh1 = data['data_samples'][batch_idx].scale_factor
                all_pred_rboxes = []
                for r_id, results_list in enumerate(all_results_list):
                    pred_boxes = results_list[batch_idx].bboxes
                    all_pred_rboxes.append(deepcopy(pred_boxes))
                all_pred_rboxes = torch.stack(all_pred_rboxes, dim=1) # N x R x 8
                gt_rboxes = deepcopy(data['data_samples'][batch_idx].gt_instances.bboxes.tensor)
                ...
                img_pth = data['data_samples'][batch_idx].img_path
...
if __name__ == '__main__':
    ...
    data_root = f'{root}/FMoW_Part{PartID}'
    img_dir = f'{data_root}/images'
    ann_dir = f'{data_root}/annotations'
    _, class_names = load_dota(ann_dir)
    main(data_root, img_dir, ann_dir, class_names, batch_size=12)
