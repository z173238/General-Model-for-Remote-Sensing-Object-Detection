...
def draw_dt_poly(img, points, cat, score, rect_color, text_color=(200, 0, 0),
    ...
    cv2.polylines(img, [p], 2, rect_color, thickness=2)
...
def draw_gt_poly(img, in_points, cat='',
                 rect_color=(0, 200, 200), text_color=(0, 200, 200),
                 with_text=True,
                 fill_poly=False):
    # points = np.array([[910, 650], [206, 650], [458, 500], [696, 500]])
    points = np.array(in_points, dtype=np.int32)
    p = points.reshape([-1, 2])
    # img = img.copy()
    if fill_poly:
        cv2.fillConvexPoly(img, p.reshape(-1, 2), rect_color)
    else:
        cv2.polylines(img, [p], 1, rect_color, thickness=1
                      )
    show_text = '%s' % (str(cat))
    if with_text:
        cv2.putText(img, show_text,
