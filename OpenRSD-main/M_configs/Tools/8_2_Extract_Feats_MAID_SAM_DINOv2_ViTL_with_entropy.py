_base_ = ['./base_orcnn.py',
          './base_settings_dior_two_stage.py']

log_processor = dict(type='LogProcessor', window_size=10, by_epoch=True)

train_class_name = ['SAMObject']

metainfo = dict(
    classes=train_class_name,
    # 注意：这个字段在最新版本中换成了小写
    palette=[(220, 20, 60),]
    # 画图时候的颜色，随便设置即可
)

# dataset settings
dataset_type = 'DOTADatasetOnline'
data_root = 'data/million_aid/'
file_client_args = dict(backend='disk')

train_pipeline = [
    dict(type='mmdet.LoadImageFromFile', file_client_args=file_client_args),
    dict(type='mmdet.LoadAnnotations', with_bbox=True, box_type='qbox'),
    dict(type='ConvertBoxType', box_type_mapping=dict(gt_bboxes='rbox')),
    dict(type='mmdet.Resize', scale=(1024, 1024), keep_ratio=True),
    dict(
        type='mmdet.RandomFlip',
        prob=0.0,
        direction=['horizontal', 'vertical', 'diagonal']),
    dict(type='mmdet.PackDetInputs')
]
train_dataloader = dict(
    batch_size=1,
    dataset=dict(
        type=dataset_type,
        data_root=data_root,
        metainfo=metainfo,
        ann_file='Resize_SAM_PS_Labels/',
        data_prefix=dict(img_path='test_png/'),
        img_shape=(1024, 1024),
        filter_cfg=dict(filter_empty_gt=True),
        pipeline=train_pipeline))

max_epochs = 1
train_cfg = dict(type='EpochBasedTrainLoop', max_epochs=max_epochs, val_interval=65534)

custom_imports = dict(
    imports=[
        'M_AD.models.detectors.Tool_feat_patch_extractor_DINOv2_with_entropy',
        'M_AD.datasets.dota_online',
    ],
    allow_failed_imports=False)


model = dict(
    type='ToolFeatExtractor',
    data_preprocessor=dict(
        type='mmdet.DetDataPreprocessor',
        mean=[123.675, 116.28, 103.53],
        std=[58.395, 57.12, 57.375],
        bgr_to_rgb=False,
        pad_size_divisor=32,
        boxtype2tensor=False),
    out_dir='/data/space2/huangziyue/million_aid/8_2_Extract_Feats_MAID_SAM_DINOv2_ViTL_with_entropy',
    bbox_roi_extractor=dict(
        out_size=(224, 224),
        spatial_scale=1.0,
        sampling_ratio=2,
        clockwise=True
    ),
    roi_scale_factor=1.25,
    roi_head=dict(
        bbox_head=dict(
            num_classes=len(train_class_name),))
)


