...
def parse_args():
    ...
    parser.add_argument('-u', help='use_teacher')
    args = parser.parse_args()
    return args

args = parse_args()
gpu = int(args.d)
use_teacher = int(args.u)
if use_teacher == 1:
    use_teacher = True
else:
    use_teacher = False
...
print('#' * 100)
print(f'epochs: {epochs}, gpu: {gpu}, use_teacher: {use_teacher}')
...
def main(tst_cfg_pth, ckpt_pth, work_dir, results_pth, eval_pth,
    ...
    #################################
    cfg.model.detector.val_support_classes = val_support_classes
    cfg.model.detector.val_dataset_flag = val_dataset_flag
    cfg.model.detector.val_using_aux = val_using_aux
    cfg.detector.val_support_classes = val_support_classes
    cfg.detector.val_dataset_flag = val_dataset_flag
    cfg.detector.val_using_aux = val_using_aux
    cfg.detector.val_using_teacher = use_teacher
...
if __name__ == '__main__':
    ...
    model_info = dict(
        val_using_aux=False,
        cfg_pth='A12_SelfTrain/A12_multiview_self_train_v4.py',
        cfg_name='A12_multiview_self_train_v4',
    )
    cfg_name = model_info['cfg_name']
    cfg_pth = model_info['cfg_pth']
    val_using_aux = model_info['val_using_aux']
    if use_teacher:
        work_root = f'./results/TEST_EVAL/Formal_12_11_{cfg_name}_UsingTeacher'
    else:
        work_root = f'./results/TEST_EVAL/Formal_12_11_{cfg_name}'
    mkdir(work_root)
    for epoch in epochs:
        print(model_info)
