...
class RVSA_MTP_branches(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=80, embed_dim=768, depth=12,
        ...
        if patch_size == 16 and len(out_indices) == 3:
        ...
        elif patch_size == 16 and len(out_indices) == 4:
            self.fpn1 = nn.Sequential(
                nn.ConvTranspose2d(embed_dim, embed_dim, kernel_size=2, stride=2),
                Norm2d(embed_dim),
                nn.GELU(),
    ...
    def forward_features(self, x):
        ...
        if self.patch_size == 16 and len(self.out_indices) == 3:
            # ops = [self.fpn1, self.fpn2, self.fpn3, self.fpn4]
            ops = [self.fpn2, self.fpn3, self.fpn4]
        elif self.patch_size == 16 and len(self.out_indices) == 4:
            ops = [self.fpn1, self.fpn2, self.fpn3, self.fpn4]
        else:
