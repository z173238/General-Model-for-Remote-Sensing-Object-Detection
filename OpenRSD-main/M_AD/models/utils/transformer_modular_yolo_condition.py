...
class TwoWayAttentionBlock(nn.Module):
    def __init__(
        ...
        self.slot_fc = nn.Identity() # nn.Linear(embedding_dim, embedding_dim)
    ...
    def forward(
        ...
        if self.with_query_self_attn:
            q = queries
            # B x C -> B x C x C
            attn_mask = torch.eq(query_labels[:, :, None], query_labels[:, None, :])
            # attn_weight = attn_weight * 2 - 1 # 1: same class, -1: diff class
        ...
        # slot_embeds = self.slot_fc(slot_embeds)
        queries = queries + mlp_out # + slot_embeds
...
class Attention(nn.Module):
    ...
    def forward(self, q: Tensor, k: Tensor, v: Tensor, attn_mask=None) -> Tensor:
        # Input projections
        q = self.q_proj(q)
        k = self.k_proj(k)
        v = self.v_proj(v)
        ...
        #     attn_weight = attn_weight[:, None, :, :].expand_as(attn)
        #     attn =  attn_weight * attn
        ##############################
        attn = torch.softmax(attn, dim=-1)
        if attn_mask is not None:
            attn_mask = attn_mask[:, None, :, :].expand_as(attn)
            attn_weights = (~attn_mask).float() * -1e9
